...
def tri_selection_anime(arr):
    ...
    for i in range(n):
        ...
        if index_min != i:
            liste_anim[i], liste_anim[index_min] = (
                liste_anim[index_min],
                liste_anim[i],
            )

        # INDISPENSABLE : On yield après chaque échange pour voir
        # l'élément minimum se placer à sa position définitive
        yield liste_anim
...
generateur = tri_selection_anime(
    ma_liste
...
plt.title(f"Visualisation en direct du Tri par Selection sur {nom_fichier}\n")
plt.show()
